...
export const getGameInfo = ( isDragonFeature:boolean, betAmount: number, isSureWin: any ) => {
    let symbols: number[] = [];
    let SYMBOLS: number[][] =[];
    ...
    if ( Number(isSureWin) === 2) {
        SYMBOLS = REEL.SUREWIN;
        if (isDragonFeature) SYMBOLS = REEL.FREESPIN;
    } else {
        if (isDragonFeature) SYMBOLS = REEL.FREESPIN;
        else SYMBOLS = REEL.NORMAL;
    }
    for (let i = 0; i < 3; i++) {
    ...
    const mulInfo = getMultiplierInfo(isDragonFeature, isSureWin);
...
const getMultiplierInfo = (isDragon: boolean, isSureWin: any ) => {
    ...
    if (isDragon) cnt = isaac.random() > 0.9 ? 3 : 4, expandingFlag = false;
        ...
        if (expandingFlag) {
            ...
            if (isSureWin) {
                if (probability > sureWinThresholds[0]) cnt = 1;//96
                else cnt = 2;
            } else {
